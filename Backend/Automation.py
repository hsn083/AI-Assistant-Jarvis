...
# Known sites and how to build a real search URL for each. Using each
# site's actual search query parameter (not just opening the homepage)
# so "find X on facebook" actually lands on facebook's search results
...
def SiteSearch(site_and_query):
    """Opens real search results on a named site, e.g. SiteSearch('facebook: islamic lab')
    opens Facebook's actual search results page for 'islamic lab', instead
    of just saying it can't access the internet."""
    try:
        if ":" in site_and_query:
            site, query = site_and_query.split(":", 1)
        else:
            # Fallback: first word is the site, rest is the query.
            parts = site_and_query.strip().split(" ", 1)
            site, query = (parts[0], parts[1]) if len(parts) > 1 else (parts[0], "")
        site = site.strip().lower()
        query = query.strip()

        if not query:
            # No specific search term -- just open the site itself.
            fallback = SITE_SEARCH_URLS.get(site, f"https://www.google.com/search?q={site}")
            webbrowser.open(fallback.split("?")[0].split("/search")[0] if site in SITE_SEARCH_URLS else fallback)
            return True

        if site in SITE_SEARCH_URLS:
            from urllib.parse import quote_plus
            url = SITE_SEARCH_URLS[site].format(query=quote_plus(query))
        else:
            # Unknown site name -- fall back to a Google "site:" search so
            # this still does something useful instead of silently failing.
            from urllib.parse import quote_plus
            url = f"https://www.google.com/search?q=site:{quote_plus(site)}+{quote_plus(query)}"

        webbrowser.open(url)
        return True
    except Exception as e:
        print(f"Error in SiteSearch: {e}")
        return False


def CreateFile(path):
    """Creates a new empty file (or folder, if the path has no extension
    and ends with a path separator) at the given path. Creates parent
    folders as needed. Does not overwrite an existing file."""
    try:
        ...
        print(f"Error in CreateFile: {e}")
        return False


def _resolve_existing_path(raw_path):
    """Tries to resolve a spoken/typed file or folder reference to a real
    path on disk. Checks, in order: the literal path as given, common
    user folders (Desktop/Documents/Downloads/Pictures) by name, and a
    case-insensitive filename search within those folders one level deep.
    ...
    if lowered in common_folders and os.path.exists(common_folders[lowered]):
        return common_folders[lowered]

    # Search one level deep in common folders for a matching filename.
...
def OpenFile(path):
    """Opens an existing file or folder with the OS default application."""
    try:
        resolved = _resolve_existing_path(path)
        if not resolved:
            print(f"Could not find file or folder: {path}")
            return False
        os.startfile(resolved)  # Windows-only, matches this project's target OS.
...
def DeleteFile(path):
    """Moves a file or folder to the Recycle Bin (not permanent deletion)
    so a misheard voice command is still recoverable. Refuses to act if
    the path can't be confidently resolved, rather than guessing."""
    try:
        resolved = _resolve_existing_path(path)
        if not resolved:
            print(f"Could not find file or folder to delete: {path}")
            return False
        try:
            from send2trash import send2trash
            send2trash(resolved)
            print(f"Moved to Recycle Bin: {resolved}")
            return True
        except ImportError:
            print("send2trash is not installed (pip install send2trash) -- refusing to "
                  "permanently delete files without a recoverable option.")
            return False
    except Exception as e:
...
async def TranslateAndExecute(commands: list[str]):
    ...
    for command in commands:
        ...
        if command.startswith("open file "):
            target = command.removeprefix("open file ").strip()
            fun = asyncio.to_thread(OpenFile, target)
            funcs.append(fun)
        elif command.startswith("create file "):
            target = command.removeprefix("create file ").strip()
            fun = asyncio.to_thread(CreateFile, target)
            funcs.append(fun)
        elif command.startswith("delete file "):
            target = command.removeprefix("delete file ").strip()
            fun = asyncio.to_thread(DeleteFile, target)
            funcs.append(fun)
            ...
            funcs.append(fun)
        elif command.startswith("site search "):
            site_and_query = command.removeprefix("site search ").strip()
            fun = asyncio.to_thread(SiteSearch, site_and_query)
            funcs.append(fun)
